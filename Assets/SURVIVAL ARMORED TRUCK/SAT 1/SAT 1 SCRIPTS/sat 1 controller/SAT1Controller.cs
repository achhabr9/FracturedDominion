...
public class SAT1Controller : MonoBehaviour
{
    ...
    void HandleSteering()
    {
        ...
        {
            rb.AddTorque(transform.up * steerInput * turnSpeed * rb.velocity.magnitude);
        }
    }
    ...
    void HandleDrifting()
    {
        ...
        {
            isDrifting = true;
            rb.velocity = Vector3.Lerp(rb.velocity, transform.forward * rb.velocity.magnitude * driftFactor, Time.fixedDeltaTime * 5);
        }
        else
        {
            isDrifting = false;
